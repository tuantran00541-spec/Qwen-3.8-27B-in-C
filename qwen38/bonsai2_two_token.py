#!/usr/bin/env python3
"""Stateful two-token Ternary Bonsai 2 27B native-C K3 executor.
...
def execute(model: Path, native_lib: Path, state_lib_path: Path,
            oracle_json: Path, work_dir: Path, output: Path, threads: int = 4):
    ...
    hidden0=base.embedding_row(model,directory,runtime,bos)
    hidden1=base.embedding_row(model,directory,runtime,token1)
    ...
    with K3Trunk(trunk,manifest_path,budget_bytes=2*max_layer,want_ring=2,max_pinned=0,prefer_direct_io=True) as reader:
        for token_index in (0,1):
            ...
            for il in range(N_LAYER):
                ...
                metas=base.layer_meta(manifest,il); p=f"blk.{il}"
    ...
    for il in range(N_LAYER):
        ...
        if m and base.over_limit(m,(2e-2,5e-3)):
            first_bad=il; break
    result={
    ...
    print("QWEN38_BONSAI2_TWO_TOKEN_PASS")
    return result
